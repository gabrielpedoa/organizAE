...
export function DailyFlowChart({ data, month, year }: Props) {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border rounded shadow">
          ...
          <p className="text-green-600 text-sm">Entradas: {formatCurrency(data.income)}</p>
          <p className="text-red-600 text-sm">Saídas: {formatCurrency(data.expense)}</p>
  ...
  const barSize = Math.max(20, Math.floor(300 / Math.max(1, activeDays.length)));
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">Apenas dias com entradas ou saídas confirmadas</p>
        </CardHeader>
        <CardContent>
          <div className="text-center py-8">
            <p className="text-muted-foreground">Nenhuma movimentação confirmada neste período ainda.</p>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <CardContent>
        <div className="w-full">
          <ResponsiveContainer width="100%" height={isMobile ? 220 : 280}>
            <BarChart data={chartData} margin={{ top: 20, right: 20, left: 0, bottom: shouldRotateLabels ? 60 : 20 }}>
              ...
              <Legend />
              <Bar dataKey="income" fill="#16a34a" name="Entradas" barSize={barSize} />
              <Bar dataKey="expense" fill="#dc2626" name="Saídas" barSize={barSize} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </CardContent>
